import { ClickerTargetStrategyType, type ClickTarget } from '../../../../src/lib/models/config'
import { Clicker } from '../../../../src/lib/utils/Clicker/Clicker'
import { ClickerEvent, ClickerEventType } from '../../../../src/lib/utils/Clicker/ClickerEvent'
...
describe('Clicker', () => {
  ...
  let clicker: Clicker
  let whilePresentTarget: ClickTarget
  let allFoundTarget: ClickTarget

  beforeEach(() => {
    clicker = new Clicker()
    clicker.addEventListener(ClickerEventType.error, handleError)
    clicker.addEventListener(ClickerEventType.beginClicking, handleBeginClickingEvent)
    clicker.addEventListener(ClickerEventType.foundElements, handleFoundElementsEvent)
    clicker.addEventListener(ClickerEventType.maxClicksReached, handleMaxClicksReachedEvent)
    clicker.addEventListener(ClickerEventType.clickedElements, handleClickedElementsEvent)
    clicker.addEventListener(ClickerEventType.endClicking, handleEndClickingEvent)
  ...
  test('emits error events for callers to listen for', async () => {
    ...
    await expect(clicker.clickEmAll([whilePresentTarget])).rejects.toEqual(error)
  ...
  describe('when handling target with strategy: whilePresent', () => {
    ...
    test('clicks first matching element until it is no longer returned', () => {
      ...
      const clickedElementsEventDetail = handleClickedElementsEvent.mock.lastCall![0].detail
      expect(clickedElementsEventDetail.count).toBe(2)
      expect(clickedElementsEventDetail.target).toEqual(whilePresentTarget)
    ...
    test('accounts for maxClicks', () => {
      ...
      const maxClicksReachedEventDetail = handleMaxClicksReachedEvent.mock.lastCall![0].detail
      expect(maxClicksReachedEventDetail.target).toEqual(whilePresentTarget)
  ...
  describe('when handling target with strategy: allFound', () => {
    ...
    test('clicks all matching elements', () => {
      ...
      const foundElementsEventDetail = handleFoundElementsEvent.mock.lastCall![0].detail
      expect(foundElementsEventDetail.count).toBe(2)
      expect(foundElementsEventDetail.target).toEqual(allFoundTarget)
      ...
      const clickedElementsEventDetail = handleClickedElementsEvent.mock.lastCall![0].detail
      expect(clickedElementsEventDetail.count).toBe(2)
      expect(clickedElementsEventDetail.target).toEqual(allFoundTarget)
    ...
    test('accounts for maxClicks', () => {
      ...
      expect(handleMaxClicksReachedEvent).toHaveBeenCalledOnce()
      const maxClicksReachedEventDetail = handleMaxClicksReachedEvent.mock.lastCall![0].detail
      expect(maxClicksReachedEventDetail.target).toEqual(allFoundTarget)
  ...
  describe('when handling targets with different strategies', () => {
    test('clicks each targeted element according to config', () => {
      ...
      vi.spyOn(document, 'querySelector').mockReturnValueOnce(whilePresentElement)
      vi.spyOn(document, 'querySelectorAll').mockReturnValueOnce([
        allFoundElement1,
        allFoundElement2
      ] as unknown as NodeListOf<HTMLElement>)
      ...
      expect(clickedElementsEventDetailWhilePresent.target).toEqual(whilePresentTarget)

      const clickedElementsEventDetailAllFound = handleClickedElementsEvent.mock.calls[1][0].detail
      expect(clickedElementsEventDetailAllFound.count).toBe(2)
      expect(clickedElementsEventDetailAllFound.target).toEqual(allFoundTarget)
